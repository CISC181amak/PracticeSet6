...
public class PersonDAL_Test {
	...
	public static void setUpBeforeClass() throws Exception {
		...
		Date dDate = null;
		try {
			dDate = new SimpleDateFormat("yyyy-MM-dd").parse("1996-01-03");
		} catch (ParseException e) {
			e.printStackTrace();
		}
		
		per1 = new PersonDomainModel();
		per1.setFirstName("Alex");
		per1.setLastName("Makinen");
		per1.setBirthday(dDate);
		per1.setCity("Newark");
		per1.setPostalCode(19702);
		per1.setStreet("100 Rhett Dr");
	}
